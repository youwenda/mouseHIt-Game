...
(function() {
   ...
   /**
	 ...
	 *  @NOTE: 重写验证方法
	 */
	MouseHit.prototype.__checkNextRolling = function(self){
	    // 如果每次打够5个正确的地鼠
	    if(self.ui.score>=self.requireScore) {
	        //不再绘制屏幕
	        clearInterval(self.drawCanvasInterval);
	        //不再随机产生地鼠
            clearInterval(self.drawMouseInterval);
            //显示再次抽奖div, 在这个DIV里控制清空所有随机抓取的图片
            my.DOM.show(my.DOM.get('overlay')); 
            my.DOM.show(my.DOM.get('nextLoding'));
            self.ui.score=0;
            //self.ui.setNumber(0);
            Audio.play('game_pass');
	    }
